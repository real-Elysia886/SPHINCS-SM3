...
- Both satisfy the current address constraint.
...
- `h=80,d=20` is attractive but needs 76 address bits, so it is not directly supported by the current reference address format.
...
3. 参数选择：提供激进方案 `224f` 和保守方案 `224f-dn`。前者最大化长度收益，后者保持 `d*n >= 544` 并满足 64 位地址约束。
4. 实验结果：签名长度分别降低 `20.14%` 和 `10.65%`，并提供 SM3 KAT、签名验签、篡改失败和 CI 自动测试。
...
7. 后续工作：实现扩展地址格式，验证 `h=80,d=20`，并把参数搜索扩展为更完整的优化框架。
